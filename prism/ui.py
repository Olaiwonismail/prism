...
def run_ui(engine):
    ...
    # --- Mic picker -----------------------------------------------------------
    ttk.Label(main, text="Microphone:").grid(row=1, column=0, sticky="w", pady=(10, 0))
    mic_combo = ttk.Combobox(
        main, values=[name for _, name in devices], state="readonly", width=42
    )
    mic_combo.grid(row=2, column=0, columnspan=2, sticky="we")
    if devices:
    ...
    # --- Level meter ----------------------------------------------------------
    ttk.Label(main, text="Input level:").grid(row=3, column=0, sticky="w", pady=(10, 0))
    gate_var = tk.StringVar(value="gate: shut")
    ttk.Label(main, textvariable=gate_var).grid(row=3, column=1, sticky="e", pady=(10, 0))
    meter = ttk.Progressbar(main, length=300, maximum=-_DB_FLOOR)
    meter.grid(row=4, column=0, columnspan=2, sticky="we")
