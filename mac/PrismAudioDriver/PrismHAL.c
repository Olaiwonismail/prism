// PrismHAL: CoreAudio server plug-in exposing the "Prism Microphone" virtual
// input device, fed by the shared-memory ring in PrismSharedRing.h (written
// by prism/ring_output.py).
//
// Forked from KraspHAL (https://github.com/pilshchikov/krasp, MIT License,
// Copyright (c) 2026 Stepan Pilshchikov) at commit bed68e1; see
// mac/vendor/ for the pristine upstream copy and license. Local changes:
// renamed identifiers/device strings, our bundle id, our ring file path and
// magic. Build + ad-hoc sign with mac/build_driver.sh.
...
#include "PrismSharedRing.h"

enum {
    kPrismObjectDevice = 2,
    kPrismObjectInputStream = 3,
    kPrismBufferFrameSize = 512
};
...
static PrismSharedRing* gRing = NULL;
static size_t gRingByteCount = sizeof(PrismSharedRing);

static AudioStreamBasicDescription StreamFormat(void) {
    AudioStreamBasicDescription format;
    memset(&format, 0, sizeof(format));
    format.mSampleRate = PRISM_RING_SAMPLE_RATE;
    ...
    format.mChannelsPerFrame = PRISM_RING_CHANNELS;
...
static AudioStreamRangedDescription StreamRangedFormat(void) {
    ...
    ranged.mSampleRateRange.mMinimum = PRISM_RING_SAMPLE_RATE;
    ranged.mSampleRateRange.mMaximum = PRISM_RING_SAMPLE_RATE;
    return ranged;
}
...
static bool ObjectExists(AudioObjectID objectID) {
    return objectID == kAudioObjectPlugInObject || objectID == kPrismObjectDevice || objectID == kPrismObjectInputStream;
}

static AudioClassID ClassForObject(AudioObjectID objectID) {
    switch (objectID) {
        case kAudioObjectPlugInObject: return kAudioPlugInClassID;
        case kPrismObjectDevice: return kAudioDeviceClassID;
        case kPrismObjectInputStream: return kAudioStreamClassID;
...
static AudioObjectID OwnerForObject(AudioObjectID objectID) {
    switch (objectID) {
        ...
        case kPrismObjectDevice: return kAudioObjectPlugInObject;
        case kPrismObjectInputStream: return kPrismObjectDevice;
...
static void OpenRingIfNeeded(void) {
    ...
    int fd = open(PRISM_RING_FILE_PATH, O_RDONLY, 0);
    if (fd < 0) {
        return;
    }
    ...
    PrismSharedRing* ring = (PrismSharedRing*)mapped;
    if (ring->magic != PRISM_RING_MAGIC || ring->version != PRISM_RING_VERSION || ring->capacityFrames != PRISM_RING_CAPACITY_FRAMES) {
...
static void ReadRing(float* destination, UInt32 frameCount) {
    ...
    PrismSharedRing* ring = gRing;
    if (ring == NULL || ring->magic != PRISM_RING_MAGIC) {
        return;
    }
    ...
    if (available > PRISM_RING_CAPACITY_FRAMES) {
    ...
    for (UInt32 frame = 0; frame < framesToCopy; frame++) {
        destination[frame] = ring->samples[(gReadIndex + frame) % PRISM_RING_CAPACITY_FRAMES];
    }
    gReadIndex += framesToCopy;
}

__attribute__((visibility("default")))
void* PrismPlugInFactory(CFAllocatorRef allocator, CFUUIDRef typeUUID) {
    (void)allocator;
...
static Boolean STDMETHODCALLTYPE HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress) {
    ...
    if (inObjectID == kPrismObjectDevice) {
        switch (inAddress->mSelector) {
    ...
    if (inObjectID == kPrismObjectInputStream) {
        switch (inAddress->mSelector) {
    ...
    switch (inAddress->mSelector) {
        ...
        case kAudioDevicePropertyModelUID:
            *outDataSize = sizeof(CFStringRef);
            return noErr;
        case kAudioObjectPropertyOwnedObjects:
            *outDataSize = (inObjectID == kAudioObjectPlugInObject || inObjectID == kPrismObjectDevice) ? sizeof(AudioObjectID) : 0;
            return noErr;
        case kAudioPlugInPropertyDeviceList:
        case kAudioDevicePropertyStreams:
        ...
        case kAudioDevicePropertyBufferFrameSizeRange:
            *outDataSize = sizeof(AudioValueRange);
            return noErr;
        case kAudioDevicePropertyStreamConfiguration:
            *outDataSize = StreamConfigurationSize(ScopeMatches(inAddress->mScope, kAudioObjectPropertyScopeInput) ? PRISM_RING_CHANNELS : 0);
            return noErr;
        case kAudioStreamPropertyVirtualFormat:
        case kAudioStreamPropertyPhysicalFormat:
    ...
    *outDataSize = 0;
    ...
    switch (inAddress->mSelector) {
        ...
        case kAudioObjectPropertyOwner:
            ...
            return noErr;
        case kAudioObjectPropertyName:
            CopyCFString(inObjectID == kPrismObjectInputStream ? CFSTR("Prism Microphone Input") : CFSTR("Prism Microphone"), inDataSize, outDataSize, outData);
            return noErr;
        case kAudioObjectPropertyModelName:
            CopyCFString(CFSTR("Prism Virtual Microphone"), inDataSize, outDataSize, outData);
            return noErr;
        case kAudioObjectPropertyManufacturer:
            CopyCFString(CFSTR("Prism"), inDataSize, outDataSize, outData);
            return noErr;
        case kAudioObjectPropertyOwnedObjects:
            if (inObjectID == kAudioObjectPlugInObject) {
                CopyObjectID(kPrismObjectDevice, inDataSize, outDataSize, outData);
            } else if (inObjectID == kPrismObjectDevice) {
                CopyObjectID(kPrismObjectInputStream, inDataSize, outDataSize, outData);
            }
            return noErr;
        case kAudioPlugInPropertyBundleID:
            CopyCFString(CFSTR("com.prism.audio.hal"), inDataSize, outDataSize, outData);
            return noErr;
        case kAudioPlugInPropertyDeviceList:
            CopyObjectID(kPrismObjectDevice, inDataSize, outDataSize, outData);
            return noErr;
        case kAudioPlugInPropertyTranslateUIDToDevice:
            if (inQualifierDataSize == sizeof(CFStringRef) && inQualifierData != NULL) {
                ...
                CopyObjectID(CFStringCompare(uid, CFSTR("com.prism.audio.microphone"), 0) == kCFCompareEqualTo ? kPrismObjectDevice : kAudioObjectUnknown, inDataSize, outDataSize, outData);
            } else {
        ...
        case kAudioDevicePropertyDeviceUID:
        case kAudioDevicePropertyModelUID:
            CopyCFString(CFSTR("com.prism.audio.microphone"), inDataSize, outDataSize, outData);
            return noErr;
        case kAudioDevicePropertyTransportType:
        ...
        case kAudioObjectPropertyControlList:
            *outDataSize = 0;
            return noErr;
        case kAudioDevicePropertyNominalSampleRate:
            CopyFloat64(PRISM_RING_SAMPLE_RATE, inDataSize, outDataSize, outData);
            return noErr;
        case kAudioDevicePropertyAvailableNominalSampleRates:
        case kAudioDevicePropertyBufferFrameSizeRange:
            if (inDataSize >= sizeof(AudioValueRange)) {
                AudioValueRange* range = (AudioValueRange*)outData;
                range->mMinimum = inAddress->mSelector == kAudioDevicePropertyBufferFrameSizeRange ? kPrismBufferFrameSize : PRISM_RING_SAMPLE_RATE;
                range->mMaximum = inAddress->mSelector == kAudioDevicePropertyBufferFrameSizeRange ? kPrismBufferFrameSize : PRISM_RING_SAMPLE_RATE;
                *outDataSize = sizeof(AudioValueRange);
            }
            return noErr;
        case kAudioDevicePropertyIsHidden:
            ...
            return noErr;
        case kAudioDevicePropertyBufferFrameSize:
            CopyUInt32(kPrismBufferFrameSize, inDataSize, outDataSize, outData);
            return noErr;
        case kAudioDevicePropertyStreamConfiguration:
            CopyStreamConfiguration(ScopeMatches(inAddress->mScope, kAudioObjectPropertyScopeInput) ? PRISM_RING_CHANNELS : 0, inDataSize, outDataSize, outData);
            return noErr;
        case kAudioDevicePropertyZeroTimeStampPeriod:
            CopyUInt32(kPrismBufferFrameSize, inDataSize, outDataSize, outData);
            return noErr;
...
static OSStatus STDMETHODCALLTYPE StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID) {
    (void)inDriver;
    (void)inClientID;
    if (inDeviceObjectID != kPrismObjectDevice) {
...
static OSStatus STDMETHODCALLTYPE GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed) {
    (void)inDriver;
    (void)inClientID;
    if (inDeviceObjectID != kPrismObjectDevice || outSampleTime == NULL || outHostTime == NULL || outSeed == NULL) {
        return kAudioHardwareBadObjectError;
    }
...
static OSStatus STDMETHODCALLTYPE DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer) {
    ...
    if (inDeviceObjectID != kPrismObjectDevice || inStreamObjectID != kPrismObjectInputStream) {
        return kAudioHardwareBadObjectError;
    }
